...
public class Level_Game_Main : Scene
{
    ...
    public override void ContentLoad(ContentManager _content, GraphicsDevice graphicsDevice)
    {
        ...
        touchsystem = new BattleMushroom.TouchSystem(true,-(height / 4f / 16f * 16f * Data_Level.sizemap[level_value - 1] - width));

        int lock_level = Data_Level.LEVEL_LOCK[level_value - 1];

        for (int i = 0; i < Data_Player.level_player.Length; i++)
        {
            Data_Player.hp[i] = Data_Player.hpd[i] + (MathHelper.Min(Data_Player.level_player[i],lock_level) / 10);
            Data_Player.attack[i] = Data_Player.attackd[i] + (MathHelper.Min(Data_Player.level_player[i],lock_level) / 20);
        }
    ...
    public override void Update(GraphicsDevice graphicsDevice, GameTime gameTime, ContentManager _content)
    {
        ...
        if (checkspawn)
        {
            if (Data_Enemy.cooldown >= (float)Data_Enemy.spawnenemy_cooldown[level_value - 1][spawnenemy])
            {
                ...
                for (int i = 0; i < Data_Enemy.enemy_load.Length; i++)
                {
                    ...
                    Data_Enemy.enemy_load[i] = new CoreMain.EnemyMain(Data_Enemy.spawnenemy[level_value - 1][spawnenemy].Value,new Vector2(((height / 4f / 16f * 16f * Data_Level.sizemap[level_value - 1]) - (height / 2f / 32f * 32f)) - height / 16f - (height / 2.5f / 32f * Data_Enemy.enemy_size_w[Data_Enemy.spawnenemy[level_value - 1][spawnenemy].Value]),(int)(height - (height / 4f / 16f * 16f + (height / 2.5f / 32f * 32f / 1.3f) + (height / 2.5f / 32f * Data_Enemy.enemy_size[Data_Enemy.spawnenemy[level_value - 1][spawnenemy].Value])))),level_value);
                    spawnenemy += 1;
        ...
        for (int i = 0; i < Data_Enemy.enemy_load.Length; i++)
        {
            ...
            {
                if (!new Rectangle((int)x + (int)Data_Enemy.enemy_load[i].pos.X + (int)(height / 2.5f / 32f * (float)Data_Enemy.enemy_hit_box_s1[Data_Enemy.enemy_load[i].enemy_value] / Data_Enemy.enemy_hit_box[Data_Enemy.enemy_load[i].enemy_value]) - (int)(height / 2.5f / 32f * 32f * Data_Enemy.enemy_hit_box_size[Data_Enemy.enemy_load[i].enemy_value]) + (int)(height / 2.5f / 32f * Data_Enemy.enemy_size_w[Data_Enemy.enemy_load[i].enemy_value]),(int)Data_Enemy.enemy_load[i].pos.Y,(int)(height / 2.5f / (float)Data_Enemy.enemy_hit_box_s1[Data_Enemy.enemy_load[i].enemy_value] * 32f / Data_Enemy.enemy_hit_box[Data_Enemy.enemy_load[i].enemy_value] + ((int)(height / 2.5f / 32f * 32f * Data_Enemy.enemy_hit_box_size[Data_Enemy.enemy_load[i].enemy_value]))),(int)(height / 2.5f / 32f * 32f) + (int)(height / 2.5f / 32f * Data_Enemy.enemy_size[Data_Enemy.enemy_load[i].enemy_value])).Intersects(new Rectangle((int)x + (int)Data_Enemy.enemy_load[i].attack.pos.X + (int)(height / 2.5f / 32f * (float)Data_Player.player_hit_box_s1[Data_Enemy.enemy_load[i].attack.player_value] / Data_Player.player_hit_box[Data_Enemy.enemy_load[i].attack.player_value]),(int)Data_Enemy.enemy_load[i].attack.pos.Y,(int)(height / 2.5f / (float)Data_Player.player_hit_box_s1[Data_Enemy.enemy_load[i].attack.player_value] * 32f / Data_Player.player_hit_box[Data_Enemy.enemy_load[i].attack.player_value]),(int)(height / 2.5f / 32f * 32f) + (int)(height / 2.5f / 32f * Data_Player.player_size[Data_Enemy.enemy_load[i].attack.player_value]))))
                {
                    Data_Enemy.enemy_load[i].attack_check = false;
                    Data_Enemy.enemy_load[i].attack = null;
                }  else
                    ...
                    if (Data.ui_button_player_1_pos.Contains(t.Position))
                    {
                        if (Data.inventory_player[0] != null)
                        {
                            if (Data_Player.cooldown[0] >= Data_Player.player_cooldown[Data.inventory_player[0].Value] && coin_game >= Data_Player.player_coin[Data.inventory_player[0].Value])
                            {
                                ...
                                {
                                    if (Data_Player.player_load[i] != null) continue;
                                    Data_Player.player_load[i] = new CoreMain.PlayerMain(Data.inventory_player[0].Value,new Vector2((int)(height / 16f),(int)(height - (height / 4f / 16f * 16f + (height / 2.5f / 32f * 32f / 1.3f) + (height / 2.5f / 32f * Data_Player.player_size[Data.inventory_player[0].Value])))));
                                    break;
                                }
                            }
                        }
                    } else if (Data.ui_button_player_2_pos.Contains(t.Position))
                    {
                        if (Data.inventory_player[1] != null)
                        {
                            if (Data_Player.cooldown[1] >= Data_Player.player_cooldown[Data.inventory_player[1].Value] && coin_game >= Data_Player.player_coin[Data.inventory_player[1].Value])
                            {
                                ...
                                {
                                    if (Data_Player.player_load[i] != null) continue;
                                    Data_Player.player_load[i] = new CoreMain.PlayerMain(Data.inventory_player[1].Value,new Vector2((int)(height / 16f),(int)(height - (height / 4f / 16f * 16f + (height / 2.5f / 32f * 32f / 1.3f) + (height / 2.5f / 32f * Data_Player.player_size[Data.inventory_player[1].Value])))));
                                    break;
                                }
                            }
                        }
                    } else if (Data.ui_button_player_3_pos.Contains(t.Position))
                    {
                        if (Data.inventory_player[2] != null)
                        {
                            if (Data_Player.cooldown[2] >= Data_Player.player_cooldown[Data.inventory_player[2].Value] && coin_game >= Data_Player.player_coin[Data.inventory_player[2].Value])
                            {
                                ...
                                {
                                    if (Data_Player.player_load[i] != null) continue;
                                    Data_Player.player_load[i] = new CoreMain.PlayerMain(Data.inventory_player[2].Value,new Vector2((int)(height / 16f),(int)(height - (height / 4f / 16f * 16f + (height / 2.5f / 32f * 32f / 1.3f) + (height / 2.5f / 32f * Data_Player.player_size[Data.inventory_player[2].Value])))));
                                    break;
                                }
                            }
                        }
                    } else if (Data.ui_button_player_4_pos.Contains(t.Position))
                    {
                        if (Data.inventory_player[3] != null)
                        {
                            if (Data_Player.cooldown[3] >= Data_Player.player_cooldown[Data.inventory_player[3].Value] && coin_game >= Data_Player.player_coin[Data.inventory_player[3].Value])
                            {
                                ...
                                {
                                    if (Data_Player.player_load[i] != null) continue;
                                    Data_Player.player_load[i] = new CoreMain.PlayerMain(Data.inventory_player[3].Value,new Vector2((int)(height / 16f),(int)(height - (height / 4f / 16f * 16f + (height / 2.5f / 32f * 32f / 1.3f) + (height / 2.5f / 32f * Data_Player.player_size[Data.inventory_player[3].Value])))));
                                    break;
                                }
                            }
                        }
                    } else if (Data.ui_button_player_5_pos.Contains(t.Position))
                    {
                        if (Data.inventory_player[4] != null)
                        {
                            if (Data_Player.cooldown[4] >= Data_Player.player_cooldown[Data.inventory_player[4].Value] && coin_game >= Data_Player.player_coin[Data.inventory_player[4].Value])
                            {
                                ...
                                {
                                    if (Data_Player.player_load[i] != null) continue;
                                    Data_Player.player_load[i] = new CoreMain.PlayerMain(Data.inventory_player[4].Value,new Vector2((int)(height / 16f),(int)(height - (height / 4f / 16f * 16f + (height / 2.5f / 32f * 32f / 1.3f) + (height / 2.5f / 32f * Data_Player.player_size[Data.inventory_player[4].Value])))));
                                    break;
                                }
                            }
                        }
